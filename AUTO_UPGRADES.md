...
- Aptos health: GET `http://127.0.0.1:${HTTP_PORT}/v1`. Nitro (Arbitrum, Plume, Robinhood), Sei, and OP Stack op-reth pins use `health_mode: block_time` (`eth_getBlockByNumber`, latest block ≤10s old); chains without YAML health config skip the check after compose up.
