#!/usr/bin/env bash
#
# Configure B² Network: create .env from env.template and set EXT_IP.
#
# Usage: ./configure.sh
#
set -euo pipefail
...
CURRENT_ADVERTISE_IP="$(grep -E '^OP_NODE_P2P_ADVERTISE_IP=' "${ENV_FILE}" | cut -d= -f2- || true)"
if [[ "${CURRENT_ADVERTISE_IP}" != "${PUBLIC_IP}" ]]; then
...
HOST_DATADIR="$(env_get HOST_DATADIR "${HOME}/b2-op-geth-data")"
HOST_OP_NODE_DATADIR="$(env_get HOST_OP_NODE_DATADIR "${HOME}/b2-op-node-data")"
mkdir -p "${HOST_DATADIR}" "${HOST_OP_NODE_DATADIR}"
chmod -R a+rX "${SCRIPT_DIR}/config"

echo ""
echo "Next:"
echo "  # L1 defaults to B² Hub public RPCs (override OP_NODE_L1_* if needed)"
echo "  ./create-jwt.sh"
echo "  ./init-database.sh   # or ./restore-snapshot.sh"
