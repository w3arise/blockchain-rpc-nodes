...
Pinned to **v6.6.3** (`SEID_VERSION`). Use **v6.4.2+** minimum — v6.4.0–v6.4.1 incorrectly pruned receipts regardless of `min-retain-blocks`. Same-series `v6.6.*` patches are **tag-only** — after a pin PR merges: `./scripts/apply-tag-only.sh sei`
