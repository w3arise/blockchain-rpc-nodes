...
If the external node fails with `Too many open files` during RocksDB catch-up, recreate it so compose `ulimits` apply: `docker compose up -d --force-recreate external-node`. The same recreate is needed after compose changes to **`stop_signal: SIGINT`** (the EN ignores Docker’s default SIGTERM).
