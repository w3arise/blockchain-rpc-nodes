#!/bin/sh
# Hemi mainnet op-geth entrypoint (snap sync / hemi-min).
# Derived from hemilabs/hemi-node scripts/gen.sh output for mainnet + snap.
set -xe
...
echo "Running Hemi op-geth (mainnet, snap)"
geth \
