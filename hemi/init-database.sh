#!/usr/bin/env bash
#
# Initialize Hemi op-geth datadir from genesis (first start only).
#
...
DATA_DIR="${HEMI_GETH_DATA:-${HOME}/hemi-op-geth-data}"
...
docker run --rm \
  ...
  "${OP_GETH_IMAGE}" \
  init --state.scheme hash --datadir /data/geth /genesis.json

echo "done. run: sudo chown -R 65532:65532 ${DATA_DIR} ${HEMI_TBC_DATA:-${HOME}/hemi-tbc-data}"
