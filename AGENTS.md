...
Linux Docker does not define `host.docker.internal` unless Compose adds it. Set it **only** on **Nitro** and **OP Stack** services (`op-node`, `op-geth` / `op-reth`, and OP alt-DA sidecars such as EigenDA that talk to L1) that are **not** `network_mode: host`:
...
op-node and Nitro need L1 (and often a beacon) RPC; pointing `.env` at `http://host.docker.internal:<port>` reaches a node on the Docker host. `127.0.0.1` inside a bridge-network container is the container, not the host.

Do **not** add this to L1 clients, ZK Stack external nodes, Morph, Cosmos/Tendermint, postgres/monitoring, or other stacks. Skip **`network_mode: host`** (use `127.0.0.1`). Host processes must still listen on `0.0.0.0` or the docker0 address — loopback-only binds stay unreachable from containers.
...
16. Follow [RPC host bind and HTTP port](#rpc-host-bind-and-http-port) and [Ports, connectivity, and P2P (L2)](#ports-connectivity-and-p2p-l2): `RPC_BIND_ADDR` + `HTTP_PORT` / `WS_PORT`, public P2P (TCP + UDP), op-node admin RPC on localhost, `configure.sh` for `EXT_IP` / `OP_NODE_P2P_ADVERTISE_IP`. Wire `OP_NODE_P2P_LISTEN_TCP_PORT` / `OP_NODE_P2P_LISTEN_UDP_PORT` from `${OP_NODE_P2P_PORT}` in compose — do not duplicate listen ports in `env.template`. Add **`extra_hosts`** on `op-node` and the execution client (and L1-talking alt-DA sidecars) unless `network_mode: host` — see [Host gateway (`extra_hosts`)](#host-gateway-extra_hosts).
...
20. Use `STATE_SCHEME=path`, `STATE_HISTORY=0`, and `--execution.caching.archive` for archive defaults (see [Arbitrum Nitro (PathDB / PBSS)](#arbitrum-nitro-pathdb--pbss)). Add **`extra_hosts`** unless `network_mode: host` — see [Host gateway (`extra_hosts`)](#host-gateway-extra_hosts).
