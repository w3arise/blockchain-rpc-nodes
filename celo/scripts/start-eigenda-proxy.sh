#!/bin/sh
# Adapted from celo-org/celo-l2-node-docker-compose scripts/start-eigenda-proxy.sh
set -e

EXTENDED_EIGENDA_PARAMETERS="${EXTENDED_EIGENDA_PARAMETERS:-}"

# Public GCS blob archive (optional; speeds catch-up for Alt-DA).
if [ -n "${EIGENDA_LOCAL_ARCHIVE_BLOBS:-}" ]; then
  EXTENDED_EIGENDA_PARAMETERS="${EXTENDED_EIGENDA_PARAMETERS} \
  --s3.credential-type=${EIGENDA_LOCAL_S3_CREDENTIAL_TYPE} \
...
exec ./eigenda-proxy --addr=0.0.0.0 \
  ...
  --eigenda.eth-rpc="${OP_NODE_L1_ETH_RPC}" \
  ...
  --eigenda.v2.eth-rpc="${OP_NODE_L1_ETH_RPC}" \
