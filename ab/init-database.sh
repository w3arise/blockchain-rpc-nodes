#!/usr/bin/env bash
#
# Initialize AB Core geth datadir from genesis (from-scratch sync).
#
# Usage:
...
DATA_DIR="${HOST_DATADIR:-${HOME}/ab-data}"
...
echo "==> Building AB Core image (if needed)"
docker compose build ab-node
...
docker compose run --rm \
  ...
  ab-node init --datadir /data /config/genesis.json
