#!/usr/bin/env bash
#
# Configure Plasma deployment: create .env from env.template, set public IP, mkdir datadirs.
#
# Usage: ./configure.sh
#
set -euo pipefail
...
mkdir -p "${EXEC_DIR}" "${CONS_DIR}" "${SCRIPT_DIR}/config"
...
echo "Next:"
echo "  ./create-jwt.sh"
echo "  ./restore-snapshot.sh   # requester-pays S3; or ./init-database.sh for genesis"
