#!/usr/bin/env bash
#
# Restore a Conduit Mode mainnet op-reth snapshot into HOST_DATADIR.
#
# Usage: ./restore-snapshot.sh
#
# Requires: gcloud CLI with an active project (requester-pays bucket).
#
set -euo pipefail
...
DATA_DIR="${HOST_DATADIR:-${HOME}/mode-op-reth-data}"
DATA_DIR="${DATA_DIR/\$HOME/$HOME}"
NETWORK_SLUG="${SNAPSHOT_NETWORK:-mode-mainnet-0}"
GCS_URI="gs://conduit-networks-snapshots/${NETWORK_SLUG}/latest.tar"
TMP_BASE="${SNAPSHOT_TMPDIR:-${HOME}/mode-snapshot-tmp}"
